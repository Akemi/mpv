...
class MacOSCommon: NSObject {

    var window: NSWindow?
    @objc var view: MetalView?
    ...
    @objc func config(_ vo: UnsafeMutablePointer<vo>) -> Bool {
        ...
        DispatchQueue.main.sync {
            ...
            window = MetalWindow(contentRect: wr, screen: targetScreen, view: view/*, cocoaCB: self*/)
            ...
            //window?.title = "title..."
            //window?.minSize = NSMakeSize(160, 90)
            //window?.collectionBehavior = .fullScreenPrimary
            //window?.contentAspectRatio = NSSize(width: 1270, height: 720)
            ...
            //view?.frame = window?.contentView?.frame ?? NSMakeRect(0, 0, 1270, 720)
        }
    ...
    @objc func control(_ vo: UnsafeMutablePointer<vo>) -> Int32 {
        return VO_NOTIMPL
    }
